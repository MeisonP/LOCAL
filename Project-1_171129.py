...
import argparse
import struct
#be used in handling binary data
import errno
import time
import os
#other library
...
import numpy as ny
import cv2
# Open Source Computer Vision Library from Opencv
#the scipy.misc/scipy.io also can do some I/O work. eg:imread....
import scipy.io
# this is a model contained in scipy, not a class.
...
def parse_args():
        ...
        parser.add_argument('--model_weights',type=str,
                            default='C:\\Users\\Mason\\desktop\\imagenet-vgg-verydeep-19.mat',
        ...
        parser.add_argument('--pooling_type',type=str,default='avg',
                           help='type of pooling in convolutional neural network.args.model_weights')
...
def get_noise_image(noise_ratio,content_img):
    ...
    noise_img=ny.random.unifom(-20.,20.,content_img.shape).astype(ny.float32)
...
def preprocess(img):
    ...
    # add a new axis in list, ny.newaxis equal to None,ima[None,:,:,:]
    img -= ny.array([123.68,116.779,103.939]).reshape((1,1,1,3))
    #numpy.array(object, dtype=None, copy=True, order=None, subok=False, ndmin=0).
    # ny.arrar([1.0,2.0,3.0]).reshape((1,1,1,3)) results transfer 
...
def build_model(input_img):  
    ...
    net['input']=tf.Variable(ny.zeros((1,h,w,d),dtype=ny.float32))
    ...
    #and the arguments conv{}_{} are initialized by tf.variable(ny.zeros())
...
def get_biases(vgg_layers,i):
    ...
    b=tf.constant(ny.reshape(bias,(bias.size)))    
...
##### V 输出 image output
def convert_to_origional_colors(content_img,stylized_img):
    ...
    dst = cv2.cvtColor(merged, inv_cvt_type).astype(ny.float32)
    dst = preprocess(dst)
    return dst
...
def postprocess(img):
    ...
    # shape(h,w,d) to (1,h,w,d)
    img=img[0]
    img=ny.clip(img,0,255).astype(ny.float32)# to uint8
    img=img.astye(ny.uint8)
    # ny.clip(array_name,Low limit,Up limit),
...
def stylize(content_img,style_imgs,init_img,frame=None):
    ...
    #optimization algorithm
            ...
            if args.origional_colors:
                output_img=convert_to_origional_colors(ny.copy(content_img),output_img)
    #output
